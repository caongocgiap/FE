...
const QuanLyHocKy = () => {
    ...
    const { listHocKy, totalPage, pageSize, tableLoading, getPaggingHocKy } = useHocKy();
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (record) => (
                <Space size="middle">
                    <button type='button' className='btn btn-success' onClick={() => showDetailModal(record)}><EyeFilled /></button>
                    <button type='button' className='btn btn-warning' onClick={() => showEditModal(record)}><EditOutlined /></button>
                </Space>
            ),
        },
    ];
        ...
        <>
            ...
            <Table 
                ...
                pagination={{
                    pageSize: pageSize,
                    total: totalPage - 5,
                    onChange: (page) => { getPaggingHocKy(page) }
                }} 
            ...
            <Modal
                ...
                maxWidth={'50%'}
            >
                <SuaHocKy data={rowData} rowKey={(record) => record.id} />
            </Modal>
        </>
    )
}
